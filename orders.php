...
<script language="javascript">
    ...
    function get_oid2(oid){
            var xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange=function() {
            if (this.readyState == 4 && this.status == 200) {
            document.getElementById("updateorder").innerHTML = this.responseText;
            }
        };
    xhttp.open("GET", "editorder.php?oid=" + oid, true);
    xhttp.send();
    }
            ...
            <form name="form1">
                <div class="panel panel-default">
                            <div class="panel-heading">
                                <h1 class="panel-title"><i class="fa fa-dollar fa-fw"></i> Orders</h1>
                            </div>
                                    ...
                                    <table class="table table-bordered table-hover table-striped">
                                        ...
                                        <tbody>
                                            ...
                                            $sql ="SELECT od.orderID,od.orderDate,od.customerID,od.Status,pd.First_Name,pd.Middle_Name,pd.Last_Name
                                                ...
                                                //   echo $sql;
                                            //$sql="SELECT od.orderID,od.productID,pd.Picture,od.Quantity,od.Price FROM tbl_orderdetail
                                            //as od INNER JOIN tbl_products as pd ON od.productID=pd.prodid";
                                            ...
                                            if($result>=1){
                                                while($row = mysqli_fetch_assoc($result))
                                                {
                                                    ...
                                                    echo "<td width='50'><button type='button' class='btn btn-primary' data-toggle='modal' data-target='#myModal2' onclick='get_oid2(".$row["orderID"].")'> Update</button></td>";
                                                    //echo "<td width='50'><a href='editorder.php?oid=".$row->orderID."' class='fa fa-edit btn btn-primary'> Update</a>";
                                                }
                                            }else{
                                                echo "<tr style='color:black';>";
 ...
 <div class="container">
  ...
  <div class="modal fade" id="myModal2" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-header" style="background-color:#337AB7;">
            <button type="button" class="close" data-dismiss="modal">&times;</button>
            <h4 class="modal-title" style="text-align:center; font-size:30px";>Update Order Status</h4>
        </div>
            <div id="updateorder">
            </div>
      </div>
